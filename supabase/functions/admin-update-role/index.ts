...
type Body = {
  userId: string;
  newRole: "admin" | "triage" | "service" | "panel";
};
...
export default async function handler(req: Request): Promise<Response> {
  ...
  const { userId, newRole } = body || ({} as Body);
  const allowedRoles = new Set(["admin", "triage", "service", "panel"]);
  if (!userId || !newRole || !allowedRoles.has(newRole)) {
    return new Response(JSON.stringify({ ok: false, error: "Missing or invalid userId/newRole" }), { status: 400 });
  }
